...
export default function Paquetes() {
  const paquetes = usePublicSectionData(() => paquetesService.getAll(), PAQUETES)
          ...
          {paquetes.map((p, i) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="font-display text-xl md:text-2xl font-semibold mb-1 text-ink">{p.nombre}</h3>
                ...
                <div className="mb-6">
                  <span className="font-display text-3xl md:text-4xl font-semibold text-gradient">{p.precio}</span>
                </div>
                ...
                <ul className="flex flex-col gap-3 mb-8 flex-1">
                  {p.incluye.map((linea) => (
                    <li key={linea} className="flex items-start gap-2.5 text-sm">
                      ...
                      <span className="text-ink/75">{linea}</span>
                    </li>
                  ))}
                </ul>
